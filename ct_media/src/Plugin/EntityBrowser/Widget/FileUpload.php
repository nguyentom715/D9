...
namespace Drupal\ct_media\Plugin\EntityBrowser\Widget;
...
use Drupal\ct_media\BundleResolverInterface;
use Drupal\ct_media\SourceFieldTrait;
use Drupal\ct_media\MediaInterface;
...
class FileUpload extends EntityFormProxy {
   ...
   *   The file system service.
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, EventDispatcherInterface $event_dispatcher, EntityTypeManagerInterface $entity_manager, WidgetValidationManager $widget_validation_manager, BundleResolverInterface $bundle_resolver, AccountInterface $current_user, Token $token, FileSystemInterface $file_system) {
  ...
  public static function create(ContainerInterface $container, array $configuration, $plugin_id, $plugin_definition) {
    ...
    return new static(
      ...
      $container->get('plugin.manager.ct_media.bundle_resolver')->createInstance($bundle_resolver),
  ...
  protected function generateEntity($input) {
    ...
    $ready = $this->fileSystem->prepareDirectory($dir, FileSystemInterface::CREATE_DIRECTORY | FileSystemInterface::MODIFY_PERMISSIONS);
    if ($ready) {
      $file = \Drupal::service('file.repository')->move($this->getFile($entity), $destination);
      if ($file) {
        $file->setTemporary();
        $file->save();
  ...
  public function processEntityForm(array $entity_form) {
    $type_config = $entity_form['#entity']->getSource()->getConfiguration();
